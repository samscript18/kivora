...
describe("InvitationEmailService", () => {
  it("sends the one-time invitation through the configured mail service", async () => {
    ...
    await expect(service.sendInvitation({ email: "manager@example.com", organizationName: "Northstar Rentals", inviterName: "Ada", role: "revenue_manager", token: "private-token", expiresAt: new Date("2026-07-29T12:00:00Z") })).resolves.toEqual({ provider: "smtp", messageId: "email_123" });

    expect(sendMail).toHaveBeenCalledWith(expect.objectContaining({
      to: "manager@example.com",
      subject: expect.stringContaining("Northstar Rentals"),
      text: expect.stringContaining("https://kivora.test/invite?token=private-token"),
