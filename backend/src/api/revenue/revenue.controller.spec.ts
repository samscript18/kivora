...
describe("RevenueController", () => {
  ...
  const revenue = { capabilities: jest.fn(() => ({ wheelhouse: { connected: true } })), preview: jest.fn(() => ({ mutated: false, source: "Wheelhouse live preview" })), underwrite: jest.fn(() => ({ source: "Wheelhouse live market report" })) };
  ...
  it("reports live capabilities", () => expect(controller.capabilities()).toEqual({ wheelhouse: { connected: true } }));
  ...
  it("delegates live underwriting", async () => expect(await controller.underwrite({ address: "1200 Brickell Bay Dr", marketId: 1, acquisitionCost: 500000, annualExpenses: 30000 })).toMatchObject({ source: "Wheelhouse live market report" }));
});
