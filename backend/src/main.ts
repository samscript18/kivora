...
async function bootstrap() {
  ...
  app.use(helmet());
  app.use((request: any, response: any, next: () => void) => { const requestId = request.headers["x-request-id"] || randomUUID(); request.headers["x-request-id"] = requestId; response.setHeader("x-request-id", requestId); next(); });
