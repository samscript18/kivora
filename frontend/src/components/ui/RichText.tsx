...
export function RichText({ text, className = "" }: { text: string; className?: string }) {
  ...
  return <div className={`space-y-2.5 text-[13px] leading-6 text-slate-300 ${className}`}>
    {blocks.map((raw, index) => {
      ...
      if (bullet) return <div key={index} className="flex gap-2.5"><span className="mt-[9px] h-1 w-1 flex-none rounded-full bg-accent"/><p>{inline(bullet[1])}</p></div>;
