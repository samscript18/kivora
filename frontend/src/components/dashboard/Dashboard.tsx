...
function Logo() {
	return (
		<div className="flex items-center gap-2.5">
			...
			<div>
				<div className="display text-[20px] font-extrabold tracking-[-.06em]">kivora</div>
				<div className="mt-[-3px] text-[8px] font-bold uppercase tracking-[.18em] text-[#78827d]">Revenue intelligence</div>
			</div>
		</div>
	);
}

function Sidebar({ mobile, onClose, active, onNavigate, onSettings }: { mobile?: boolean; onClose?: () => void; active: WorkspaceView; onNavigate: (view: WorkspaceView) => void; onSettings: () => void }) {
	const nav = [
	...
	return (
		<aside className={`${mobile ? "fixed inset-y-0 left-0 z-50 shadow-2xl" : "desktop-only fixed inset-y-0 left-0"} flex w-[248px] flex-col border-r border-[#dfe4e0] bg-[#f9faf9] px-4 py-5`}>
...
function Metric({ label, value, detail, icon: I, tone }: { label: string; value: string; detail: string; icon: any; tone: string }) {
	return (
		<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card rounded-2xl p-5">
			...
			<div className="display mt-3 text-[26px] font-bold tracking-[-.04em]">{value}</div>
			<div className="mt-1 text-[10px] text-[#8a938e]">{detail}</div>
		</motion.div>
	);
}

function IncidentPanel({ incident, onOpen }: { incident: any; onOpen: () => void }) {
	return (
		<motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} className="overflow-hidden rounded-2xl border border-[#edcfc6] bg-[#fffaf8]">
			<div className="flex flex-col gap-5 p-5 sm:flex-row sm:items-center">
				...
				<div className="min-w-0 flex-1">
					<div className="flex items-center gap-2">
						<span className="rounded-full bg-[#f7dfd8] px-2 py-1 text-[9px] font-bold uppercase tracking-[.1em] text-[#9d402f]">Critical incident</span>
						<span className="text-[10px] text-[#929993]">{incident.detectedAt}</span>
					</div>
					<h2 className="display mt-2 text-[17px] font-bold tracking-[-.025em]">{incident.title}</h2>
				...
				<div className="flex items-center gap-7 sm:border-l sm:border-[#ecdcd6] sm:pl-7">
					<div>
						<div className="text-[9px] font-bold uppercase tracking-[.1em] text-[#9c8178]">Revenue at risk</div>
						<div className="display mt-1 text-xl font-bold text-[#9e402e]">{money(incident.revenueAtRisk)}</div>
					</div>
					<button onClick={onOpen} className="flex items-center gap-2 rounded-xl bg-[#173f2e] px-4 py-3 text-[11px] font-bold text-white transition hover:bg-[#20583e]">
...
export function Dashboard() {
		...
		return (
			<div className="grid min-h-screen place-items-center bg-[#030303] p-6">
				<div className="glass-card max-w-lg rounded-2xl p-8 text-center">
					...
					<p className="mt-3 text-sm text-red-400">{error instanceof Error ? error.message : "The revenue API did not return live data."}</p>
					<p className="mt-3 text-xs text-slate-500">Verify the backend, MongoDB, and Wheelhouse credentials. Kivora never substitutes synthetic data.</p>
					<button onClick={() => setSettingsOpen(true)} className="mt-5 rounded-xl border border-white/10 px-4 py-3 text-xs font-bold">
						Telegram settings
					</button>
				</div>
	...
	return (
		<div className="kivora-dashboard min-h-screen bg-[#030303]">
			<Sidebar active={active} onNavigate={setActive} onSettings={() => setSettingsOpen(true)} />
			{menu && <Sidebar mobile active={active} onNavigate={setActive} onClose={() => setMenu(false)} onSettings={() => setSettingsOpen(true)} />}
			<main className="min-h-screen md:ml-[248px]">
				<header className="sticky top-0 z-30 flex h-[70px] items-center border-b border-[#e0e5e1] bg-[#f4f6f4]/90 px-4 backdrop-blur-xl sm:px-7">
					...
					<div>
						<h1 className="display text-[17px] font-bold tracking-[-.03em]">{active}</h1>
						<div className="mt-0.5 flex items-center gap-2 text-[9px] text-[#84908a]">
							<span className="h-1.5 w-1.5 rounded-full bg-[#2d9768]" /> Live via Wheelhouse
						</div>
					</div>
				...
				{active === "Overview" ? (
					<div className="mx-auto max-w-[1440px] p-4 sm:p-7">
						<div className="mb-6">
							<div className="text-[11px] text-[#77817c]">{new Date().toLocaleDateString(undefined, { dateStyle: "full" })}</div>
						...
						<div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
							...
							<Metric label="Revenue at risk" value={money(s.atRisk)} detail="Detected live incidents" icon={AlertTriangle} tone="bg-[#fbe6e0] text-[#aa4935]" />
							<Metric label="Opportunities" value={String(s.opportunities)} detail="Derived from open incidents" icon={TrendingUp} tone="bg-[#e5ebf5] text-[#49678f]" />
							<Metric label="30-day occupancy" value={`${s.occupancy}%`} detail="Wheelhouse KPI average" icon={Gauge} tone="bg-[#eee8f8] text-[#73579a]" />
						</div>
						<div className="mt-5">
							{data.incident ? (
						...
						<div className="mt-5 grid gap-5 xl:grid-cols-[1.5fr_1fr]">
							<section className="card rounded-2xl p-5">
								<div className="flex items-center justify-between">
									<div>
										<h3 className="display text-[14px] font-bold">Portfolio revenue</h3>
										<p className="mt-1 text-[10px] text-[#8a938e]">Actual versus local market benchmark</p>
									</div>
									<button className="rounded-lg border border-[#e2e6e3] px-3 py-2 text-[10px] font-semibold">
										Last 7 days <ChevronDown className="ml-1 inline" size={11} />
									</button>
								</div>
										...
										<AreaChart data={data.trend}>
											<defs>
												<linearGradient id="rev" x1="0" y1="0" x2="0" y2="1">
													<stop offset="0" stopColor="#1b6b4a" stopOpacity=".24" />
													<stop offset="1" stopColor="#1b6b4a" stopOpacity="0" />
												</linearGradient>
											</defs>
											<CartesianGrid vertical={false} stroke="#edf0ed" />
											<XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: "#89938d" }} />
											<Tooltip formatter={(v) => money(Number(v))} contentStyle={{ borderRadius: 12, border: "1px solid #e2e6e3", fontSize: 11 }} />
											<Area type="monotone" dataKey="revenue" stroke="#1b6b4a" strokeWidth={2.2} fill="url(#rev)" />
											<Area type="monotone" dataKey="market" stroke="#aeb7b1" strokeDasharray="4 4" fill="transparent" />
										</AreaChart>
									</ResponsiveContainer>
								</div>
							</section>
							<section className="card rounded-2xl p-5">
								<div className="flex items-center justify-between">
									<div>
										<h3 className="display text-[14px] font-bold">Top opportunities</h3>
										<p className="mt-1 text-[10px] text-[#8a938e]">Ranked by recoverable revenue</p>
									</div>
								...
								<div className="mt-4 divide-y divide-[#edf0ed]">
									{data.opportunities.map((o: any, i: number) => (
										<div key={o.property} className="flex items-center gap-3 py-3.5">
											<div className="grid h-8 w-8 place-items-center rounded-lg bg-[#edf2ee] text-[10px] font-bold text-[#446052]">0{i + 1}</div>
											<div className="min-w-0 flex-1">
												<div className="truncate text-[11px] font-bold">{o.property}</div>
												<div className="mt-1 truncate text-[9px] text-[#87908b]">
													{o.action} · {o.confidence}% confidence
												</div>
											</div>
											<div className="text-right">
												<div className="text-[11px] font-bold text-[#1b6b4a]">+{money(o.impact)}</div>
												<span className="text-[8px] uppercase text-[#969f99]">{o.tag}</span>
											</div>
										</div>
									))}
								</div>
						...
						<section className="card mt-5 rounded-2xl p-5">
							<div className="flex items-center justify-between">
								<div>
									<h3 className="display text-[14px] font-bold">Kivora activity</h3>
									<p className="mt-1 text-[10px] text-[#8a938e]">Autonomous actions and portfolio checks</p>
								</div>
								<span className="flex items-center gap-2 text-[9px] font-bold text-[#47705d]">
									<span className="live-dot h-1.5 w-1.5 rounded-full bg-[#2a8e61]" /> MONITORING
								</span>
							</div>
							<div className="mt-4 grid gap-3 md:grid-cols-3">
								{data.activity.map((a: any) => (
									<div key={a.title} className="flex items-start gap-3 rounded-xl border border-[#e7eae8] p-3.5">
										...
										<div>
											...
											<div className="mt-1 text-[9px] text-[#8a938e]">{a.meta}</div>
										</div>
										<span className="ml-auto text-[9px] text-[#a0a8a3]">{a.time}</span>
									</div>
								))}
							</div>
						</section>
...
function IncidentDrawer({ open, onClose, incident }: { open: boolean; onClose: () => void; incident: any }) {
	...
	const fix = useMutation({
		...
		onSuccess: (result) => {
			...
			toast.success("Dynamic pricing restored", { description: `${money(result.recovered)} in projected revenue protected.` });
		},
		onError: (error) => toast.error(error instanceof Error ? error.message : "Live resolution failed"),
	});
	return (
		<AnimatePresence>
			{open && (
				<>
					<motion.div className="fixed inset-0 z-40 bg-[#13251d]/30 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} />
					<motion.aside
						initial={{ x: "100%" }}
						animate={{ x: 0 }}
						exit={{ x: "100%" }}
					...
					>
						<div className="sticky top-0 z-10 flex h-16 items-center border-b border-[#e3e7e4] bg-[#fbfcfb]/95 px-5 backdrop-blur">
							...
							<button onClick={onClose} className="ml-auto grid h-8 w-8 place-items-center rounded-lg border border-[#e1e6e2]">
								<X size={15} />
							</button>
						</div>
						<div className="p-5 sm:p-7">
							<div className="flex items-center gap-2">
								<span className="rounded-full bg-[#f8e2dc] px-2.5 py-1 text-[9px] font-bold text-[#9f402d]">CRITICAL</span>
								<span className="text-[10px] text-[#929b96]">Detected {incident.detectedAt}</span>
							</div>
							<h2 className="display mt-4 text-[25px] font-bold leading-tight tracking-[-.045em]">{incident.title}</h2>
							...
							<div className="mt-6 grid grid-cols-3 overflow-hidden rounded-2xl border border-[#e5e8e6] bg-white">
								<div className="p-4">
									<div className="text-[8px] font-bold uppercase tracking-wider text-[#929a96]">Current rate</div>
									<div className="mt-2 text-lg font-bold text-[#9f432f]">{money(incident.currentRate)}</div>
								</div>
								<div className="border-x border-[#e8ebe9] p-4">
									<div className="text-[8px] font-bold uppercase tracking-wider text-[#929a96]">Wheelhouse</div>
									<div className="mt-2 text-lg font-bold text-[#1b6b4a]">{money(incident.recommendedRate)}</div>
								</div>
								<div className="p-4">
									<div className="text-[8px] font-bold uppercase tracking-wider text-[#929a96]">At risk</div>
									<div className="mt-2 text-lg font-bold">{money(incident.revenueAtRisk)}</div>
								</div>
							</div>
							{stage === "resolved" ? (
								...
								<>
									<div className="mt-7">
										<div className="flex items-center gap-2 text-[11px] font-bold">
											<Bot size={15} className="text-[#1b6b4a]" /> Kivora’s diagnosis
										</div>
										<div className="soft-grid mt-3 rounded-2xl border border-[#dfe6e1] bg-[#f6f9f7] p-5">
											<div className="flex items-center gap-2">
												...
												<div>
													<div className="text-[9px] uppercase tracking-wider text-[#7d8982]">Root cause</div>
													<div className="text-[11px] font-bold">{incident.cause}</div>
												</div>
												<span className="ml-auto text-[10px] font-bold text-[#1b6b4a]">{incident.confidence}% confidence</span>
											</div>
											<p className="mt-4 text-[12px] leading-relaxed text-[#64716a]">{incident.explanation}</p>
										</div>
									</div>
									<div className="mt-7">
										<div className="text-[11px] font-bold">Evidence from Wheelhouse</div>
										<div className="mt-3 space-y-2">
											{incident.factors.map((f: any) => (
												<div key={f.label} className="flex items-center rounded-xl border border-[#e5e9e6] bg-white p-3">
													<span className="text-[10px] text-[#707a75]">{f.label}</span>
													<span className="ml-auto text-[11px] font-bold">{f.value}</span>
													<span className="ml-2 w-28 text-right text-[9px] text-[#98a09c]">{f.note}</span>
												</div>
											))}
										</div>
									</div>
									{stage === "previewing" && (
										<div className="mt-7 rounded-2xl border border-[#dce4df] bg-white p-5">
											<div className="flex items-center gap-3">
												<span className="live-dot h-2 w-2 rounded-full bg-[#1b6b4a]" />
												<div>
													<div className="text-[11px] font-bold">Requesting live Wheelhouse preview</div>
													<div className="mt-1 text-[9px] text-[#84908a]">Comparing current and proposed rates without mutating preferences.</div>
												</div>
											</div>
										</div>
									)}
									{stage === "result" && previewResult && (
										...
										>
											...
											<div className="grid grid-cols-3 p-4 text-center">
												<div>
													<div className="text-[8px] uppercase text-[#87927e]">Recovery</div>
													<div className="mt-1 text-lg font-bold text-[#32623f]">+{money(previewResult.projectedRecovery)}</div>
												</div>
												<div>
													<div className="text-[8px] uppercase text-[#87927e]">Current</div>
													<div className="mt-1 text-sm font-bold">{money(previewResult.currentRevenue)}</div>
												</div>
												<div>
													<div className="text-[8px] uppercase text-[#87927e]">Optimized</div>
													<div className="mt-1 text-sm font-bold">{money(previewResult.optimizedRevenue)}</div>
												</div>
											</div>
										</motion.div>
									)}
									<div className="mt-8 border-t border-[#e5e8e6] pt-5">
										{stage === "analysis" && incident.canPreview !== false ? (
											...
											>
												<ShieldCheck size={15} /> {fix.isPending ? "Restoring…" : "Approve & restore dynamic pricing"}
											</button>
										) : null}
										<p className="mt-3 text-center text-[9px] text-[#929b96]">The preview is live and non-mutating. Approval performs the real Wheelhouse update.</p>
									</div>
								</>
							)}
						</div>
...
function Resolved({ incident }: { incident: any }) {
	return (
		<motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="mt-7">
			<div className="rounded-2xl bg-[#173f2e] p-6 text-white">
				...
				<p className="mt-2 text-[11px] leading-relaxed text-white/65">Dynamic pricing was restored and verified for {incident.listing}. Wheelhouse synchronization completed.</p>
				<div className="mt-5 flex items-end justify-between border-t border-white/10 pt-5">
					<span className="text-[9px] uppercase tracking-wider text-white/50">Projected recovery</span>
					<span className="text-2xl font-bold text-[#d8f45b]">{money(incident.revenueAtRisk)}</span>
				</div>
			</div>
		</motion.div>
	);
